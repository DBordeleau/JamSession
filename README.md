...
> **Current status:** PRs 01–17, the five-slice $0 audio-delivery optimization, MIDI-01–MIDI-07, and STUDIO-01–STUDIO-05 are complete. The repository includes frozen MIDI/manifest/session contracts, deterministic sample-free presets, accessible shared My stems/Studio composition and recording, immutable versions, exact project import/publication/preview/export, complete MIDI collaboration paths, a tested reversible source-admission control that remains enabled, the canonical project-independent Studio shell with bounded browsing, creation, and recovery-safe switching, and a unified audio/MIDI arranger with deterministic accessible arrangement mutation, atomic finalize-and-apply, and exact multi-clip persistence. STUDIO-06 is next and owns the final parity/audio-lock gate before PR 18 resumes.
...
STUDIO-01 implemented `/studio` as the authenticated start center and `/studio/{projectId}` as the independently authorized selected session, with the nested URL retained only as a compatibility redirect. STUDIO-02 adds the bounded project browser, shared in-shell/project-page creation contract, and generation-aware save/recovery/disposal coordinator for serial switching. STUDIO-03 replaces the form-like MIDI surface with a shared audio/MIDI ruler, lanes, channel strips, selection, and inspector. STUDIO-04 adds validated pointer/keyboard clip and track commands, bounded session undo/redo, semantic autosave, and exact multi-clip collaboration round trips. STUDIO-05 reuses the piano roll/recorder in project context and atomically freezes and adds/replaces one exact clip. STUDIO-06 owns parity and final source-admission lock enablement. See the [roadmap](docs/ROADMAP.md) and [studio-forward plan](docs/studio-forward-refactor-plan.md).
