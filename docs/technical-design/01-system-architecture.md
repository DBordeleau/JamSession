...
Jam Session is a Next.js App Router application backed by Supabase Auth and Postgres. Supabase Storage is used only for private avatar originals and sanitized public avatar derivatives. Vercel is the intended host. PIVOT-09 did not mutate hosted state; PIVOT-10 is approved to destructively reset the existing hosted project to the clean baseline without changing its project reference or API keys.
