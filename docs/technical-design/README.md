...
- 17 hosted repository migrations through AVATAR-01, the pending guarded AVATAR-03 contraction, deterministic MIDI-only infrastructure seed, versioned RELEASE-02 beta definitions, current generated types, and pgTAP coverage; and
...
Wave A, LIB-01 through LIB-03, CHALLENGE-01 through CHALLENGE-03, BADGE-01, RELEASE-01 through RELEASE-03, and AVATAR-01 through AVATAR-03 are complete in the repository. The invite-only beta runs at `https://open-midi.vercel.app/` against the retained Supabase project with browser-only MIDI synthesis and deterministic local generated avatars. Later schema, data, provider, or deployment mutations continue to require an exact target check and explicit authority.
