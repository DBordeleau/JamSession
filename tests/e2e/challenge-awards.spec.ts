...
test.describe("challenge profile awards", () => {
  ...
  test("shows exact current-result awards signed out and replaces presentation after correction", async ({
    ...
    await expect(resultLink).toHaveAttribute(
      "href",
      `/challenges/${fixture.slug}?result=${fixture.firstResultId}#entry-${fixture.ids.actorEntry}`,
    );
...
commit;`);
    ...
    await expect(
      ...
      "href",
      `/challenges/${fixture.slug}?result=${correctedResultId}#entry-${fixture.ids.actorEntry}`,
    );
