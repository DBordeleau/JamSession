...
- Keep global navigation request-bounded: shared-header and authenticated primary-navigation links start with prefetch disabled and restore the Next.js default only after pointer or keyboard intent; always-visible shared-footer links remain no-prefetch. Do not reintroduce viewport fanout, hide the signed-out-first shell while claims resolve, or turn navigation into an authorization boundary.
